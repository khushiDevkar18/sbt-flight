...
import SearchFlight from "../legacy/SearchFlight";
import Swal from "sweetalert2";
import Cookies from "js-cookie";
import IconLoader from "../legacy/IconLoader";
...
const NewHome = () => {
  ...
  const validateFormCab = () => {
    ...
    if (selectedCabType === "Round Trip (Outstation)") {
      ...
      if (selectedCabInput === selectedDropCity)
        newErrors.dropCity = "Drop city must be different";
      if (!selectedDate) newErrors.pickupDate = "Pickup date is required";
      if (!dropDate) newErrors.dropDate = "Drop date is required";
      if (dropDate && selectedDate && dropDate < selectedDate)
        newErrors.dropDate = "Drop date cannot be before pickup date";
      if (!selectedTime) newErrors.pickupTime = "Pickup time is required";
    }
    ...
    if (selectedCabType === "MultiCity (Outstation)") {
      if (!selectedCabInput) newErrors.pickupCity = "Pickup city is required";
      if (stopInputs.some((input) => !input))
        newErrors.stops = "All stops must be filled";
      if (
        new Set([selectedCabInput, ...stopInputs]).size !==
        stopInputs.length + 1
      )
        newErrors.duplicate = "Duplicate cities not allowed";
      if (!selectedDate) newErrors.pickupDate = "Pickup date is required";
      if (!dropDate) newErrors.dropDate = "Drop date is required";
      if (!selectedTime) newErrors.pickupTime = "Pickup time is required";
    }
    ...
    try {
      ...
      if (data.success === "false") {
        ...
        if (selectedCabType === "Round Trip (Outstation)") {
          ...
          Cities = [, ...stopInputs].filter(Boolean);
          SearchCities = [selectedCabInput, ...stopInputs].filter(Boolean);
        }

        // Now safely create the header
        const header = {
          pickup_city: selectedCabCity,
          pickup_date: formattedPickupDate,
          ...(Cities.length > 0 && { Cities }),
          ...(SearchCities.length > 0 && { SearchCities }), // Add only if there are cities
          numberOfCities: Cities.length, // ✅ Now Cities is available
  ...
  // ******************************************************************************** Bus Module ***********************************************************************************************
  const [selectedInputBus, setSelectedInputBus] = useState("");
  const [cityListBus, setCityListBus] = useState([]);
  const [filteredCitiesBus, setFilteredCitiesBus] = useState([]);
  const [showDropdownBus, setShowDropdownBus] = useState(false);
  ...
  const [showTravelDateDropdown, setShowTravelDateDropdown] = useState(false);
  ...
  const validateFormBus = () => {
    ...
    if (!selectedInputBus || !selectedInputBus.trim()) {
      ...
      isValid = false;
    } else if (!cityListBus.some(city => city.CityName === selectedInputBus)) {
    ...
    if (!toCityInput || !toCityInput.trim()) {
      ...
      isValid = false;
    } else if (!toCityList.some(city => city.CityName === toCityInput)) {
      newErrors.toCityError = "Please select a valid city from the list";
      isValid = false;
    } else if (toCityInput === selectedInputBus) {
  ...
  const handleBusSearch = async (e) => {
    ...
    try {
      ...
      const formData = new URLSearchParams();
      formData.append('DateOfJourney', formattedDate);
      // formData.append('OriginId', selectedFromCityId);
      ...
      if (data.success === 'true') {
        const header = {
          ...
          FromCity: { CityId: selectedFromCityId, CityName: selectedInputBus },
          ToCity: { CityId: selectedToCityId, CityName: toCityInput },
        };

        // Navigate to SearchBus with data
        navigate('/SearchBus', { state: { searchHeader: header, busResults: data } });
      } else {
        setError(data.ErrorMessage || "No buses found");
      }
    } catch (err) {
      setError("API error");
      console.error(err);
    ...
    <div className="Home_page_Container">
      ...
      <div className="main_wrapper">
        <div className="module_container">
          ...
          >
            <img
              ...
              style={{
                filter:
                  activeForm === "flight" ? "grayscale(0%)" : "grayscale(100%)",
                tintColor: activeForm === "flight" ? "blue" : "black",
              }}
            />
            <span
          ...
          >
            <img
              ...
              style={{
                ...
                //     ? "grayscale(0%)"
                //     : "grayscale(100%)",
                tintColor: activeForm === "hotel" ? "blue" : "black",
              }}
            />
            <span
              className="text-lg font-semibold"
          ...
          >
            <img
              ...
              style={{
                ...
                //     ? "grayscale(0%)"
                //     : "grayscale(100%)",
                tintColor: activeForm === "cab" ? "blue" : "black",
              }}
            />
            <span
              className="text-lg font-semibold"
          ...
          >
            <img
              src={
                activeForm === "bus"
                  ? "../img/Bus_Hover.svg" // Image to show when activeForm is "flight"
                  : "../img/Bus_icon.svg" // Default image
              }
              // src="../img/Hotel-02.png"
              alt="Hotel Image"
              className="w-6 h-6"
              style={{
                ...
                //     ? "grayscale(0%)"
                //     : "grayscale(100%)",
                tintColor: activeForm === "bus" ? "blue" : "black",
              }}
            />
            <span
              className="text-lg font-semibold"
        ...
        <div className="main_container">
                              ...
                              >
                                {companies
                                  .filter((company) =>
                                    company.corporate_name
                                      .toLowerCase()
                                      .includes(inputCompany.toLowerCase())
                                  )
                                  .map((company) => (
                                    <li
                                      key={company.id}
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const errorElement =
                                document.querySelector(".redorigin");
                              if (value.trim() !== "") {
                          ...
                          <div className="airport-name">
                            {inputOrigin.split(")").slice(1).join(")").trim()}{" "}
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              // Hide error if value is not empty
                              const errorElement =
                                document.querySelector(".redestination");
                              if (value.trim() !== "") {
                                errorElement.style.display = "none";
                              } else {
                                errorElement.style.display = "block";
          ...
          {activeForm === "hotel" && (
            <form className="hotel-form " onSubmit={handleHotelSearch}>
              <div className=" hotel-box ">
                <div className="hotel-container flex flex-cols ">
                          ...
                          {filteredCompany.length > 0 ? (
                            filteredCompany.map((company, index) => (
                              <div
                                ...
                                onClick={() =>
                                  handleSelectCompany(company.corporate_name)
                                }
                              >
                  ...
                  <div className="from-hotel-group">
                    <div className="location-headers">City Name</div>
                    <div className="location-details relative">
                      <input
                        type="text"
                        className="w-full rounded-lg px-3 py-2 focus:outline-none hotel-city-name"
                      ...
                      {showDropdown1 && filteredCities.length > 0 && (
                  ...
                  {/* Check-In Date */}
                  <div className="from-hotel-group">
                    <div className="location-headers">Check-In</div>
                    <div
                      className="input-a"
                      // style={{
                      //   border: "none",
                    ...
                    >
                      <DatePicker
                        ...
                        onClickOutside={() => setCheckInIsOpen(false)} // Close calendar when clicking outside
                      />
                      <span
                        className="date-icon"
                        onClick={(e) => {
                  ...
                  {/* Check-Out Date */}
                  <div className="from-hotel-group">
                    <div className="location-headers">Check-Out</div>
                  ...
                  <div className="from-hotel-group">
                    <div className="location-headers">Rooms & Guests</div>
              ...
              <div className=" cab-box ">
                <div className="flex gap-4">
                  ...
                  {[
                    ...
                    >
                      ...
                      />
                      <span className="text-sm cursor-pointer">{type}</span>
                    </div>
                  ))}
                </div>
                {selectedCabType === "Local" ? (
                  <from className="cab-container flex flex-cols ">
                    <div className="from-cab-group">
                      ...
                      <div className="location-details relative">
                        <input
                          ...
                          onBlur={() =>
                            setTimeout(() => setShowDropdownCab(false), 200)
                          }
                          disabled={isLoading}
                        />
                    ...
                    <div className="from-cab-group">
                      <div className="cab-headers">Pickup Date</div>
                      <div className="location-details relative">
                        <DatePicker
                          className="w-full rounded-lg px-3 py-2 focus:outline-none hotel-city-name"
                          ...
                          onBlur={() =>
                            setTimeout(() => setShowDropdown5(false), 200)
                          }
                    ...
                    <div className="from-cab-group">
                      <div className="cab-headers">Pickup Time</div>
                      <div className="location-details relative">
                        <DatePicker
                          className="w-full rounded-lg px-3 py-2 focus:outline-none hotel-city-name"
                      ...
                      <div className="from-cab-group">
                        ...
                        <div className="location-details relative">
                          <input
                            ...
                            onBlur={() =>
                              setTimeout(() => setShowDropdownDrop(false), 200)
                            }
                            disabled={isLoading}
                          />
                      ...
                      {roundTripStopInputs.map((value, index) => (
                        <div key={index} className="relative mb-2 mt-2">
                          <input
                            type="text"
                            ref={(el) =>
                              (roundTripInputRefs.current[index] = el)
                            }
                            value={value}
                            onChange={(e) =>
                              handleRoundTripInputChange(index, e.target.value)
                            }
        ...
        <div className="content_below">
          <h2>
            Why choose <span className="Home_Page_Text">CoTrav?</span>
          </h2>
          <p className="SubTitle">Everything You Need, All in One Place.</p>
          <p className="subText">
            Manage every step of your business travel with a <br /> single,
            seamless platform.
          </p>
          ...
          <div className="features">
            <div className="feature_box">
              ...
              <p className="text-sm">
                With 10 years of team experience, we have built and upgraded a
                reliable technology.
              </p>
            </div>
            <div className="feature_box">
              ...
              <p className="text-sm">
                With our problem-solving approach, we provide support to you at
                every step.
              </p>
            </div>
            <div className="feature_box">
            ...
            <div className="feature_box">
              ...
              <h4 className="Home_Page_Text text-sm">
          ...
          <div className="mt-5">
            <h2>
              Services <span className="Home_Page_Text"> We Offer</span>
            </h2>
            <p className="subText">
