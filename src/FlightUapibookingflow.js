...
const FlightUapibookingflow = () => {
        ...
        try {
          ...
          if (Data.status) {
            ...
            if (responseData?.AirPriceResult?.OptionalServices) {
              const Optionalservice = responseData?.AirPriceResult?.OptionalServices;
            ...
            if (responseData?.priceRequestResponse?.AirPriceResult?.OptionalServices?.Baggage) {
              const baggagedata = responseData.priceRequestResponse?.AirPriceResult?.OptionalServices?.Baggage;
  ...
  const fetchCancellationPolicy = async () => {
    try {
      const response = await fetch(`${CONFIG.MAIN_API}/api/flights/getCancellationDateChangePolicy`, {
        method: 'GET',
        headers: {
...
useEffect(() => {
  const fetchCountryData = async () => {
    try {
      const response = await fetch('https://corporate.taxivaxi.com/api/getAllCountries');
      const result = await response.json();

      if (result.success === "1" && Array.isArray(result.response.Countries)) {
  ...
  const handleSavePassenger = () => {
    ...
    passengerList.forEach((passenger, index) => {
      ...
      const passengerData = {
        type: passenger.type,
        prefix: form.querySelector('select[name="adult_prefix[]"]')?.value || '',
        firstName: firstNameInput?.value || '',
        lastName: lastNameInput?.value || '',
        dob: dobInput?.value || '',
        age: calculateAge(dobInput?.value),
        email: emailInput?.value || '',
        contact: phoneNumbers || `+91${TaxivaxiPassengeDetails[0].employee_contact}` || '',
        gender: genderValue,
        flyerName: form.querySelector('input[name="flyername"]')?.value || '',
        flyerNumber: form.querySelector('input[name="flyernumber"]')?.value || '',
        code: matchCode,
      ...
      if (passenger.type === "Adult" && !firstAdultEmail && passengerData.email && passengerData.contact) {
  ...
  const handleSaveMeal = () => {
    ...
    const formattedData = PassengerData.map((passenger, index) => {
      const passengerMeals = selectedMeals.filter(m => m.passengerIndex === index);
...
const validateForm = () => {
  ...
  const ContinueBooking = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${base_url}makeFinalPriceRequest`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      const Data = await response.json();
        ...
        if (typeof onward === "string") {
          Swal.fire({
            title: "Error",
            text: typeof onward === "string" ? onward : "An unexpected error occurred during booking.",
            iconHtml: '<i class="fa fa-exclamation" aria-hidden="true"></i>',
            confirmButtonText: "Try Again",
          }).then((result) => {
            if (result.isConfirmed) {
      ...
      if (Data.status == true) {
        ...
        navigate("/UapibookingCompleted", { state: { FlightBooking, responseData } })
      }
                    ...
                    <div className="sp-page-p">
                      <div className="booking-left">
                        <div>
                          <div className="baggagae_policy">
                            ...
                            {Segments.map((data, index) => (
                              <div key={index} className='row' style={{ border: '1px solid #e3e3e3', margin: '0% 0%' }}>
                                  ...
                                  <div>
                                    <div className='row accordionfarename apiairportname'>
                                      <div className="airport-flight-row">
                                        <div className="airport-flight-container">
                                          ...
                                          {/* Cabin Class */}
                                          <div className="airport-cabin-class">{responseData?.CabinClass}</div>
                                        </div>
                                      </div>
                                    </div>
                                    <div className="flight-details-container">
                                      <div className='row accordionfarename apiairportname'>
                                        ...
                                        <span className='airport'>{data?.Origin?.Airport?.AirportName} {data?.Origin?.Airport?.Terminal}</span>
                                      </div>
                                      <div className='row accordionfarename apiairportname'>
                                      ...
                                      <div className='row accordionfarename apiairportname'>
                                        ...
                                        <span className='airport'>{data?.Destination?.Airport?.AirportName} {data?.Destination?.Airport?.Terminal}</span>
                                      </div>
                                      <div className='baggage-info'>
                                        <span className="cabin-baggage">
                                          <img src="/img/cabin_bag.svg" alt="Cabin Baggage" className="baggage-icon" /><strong>Cabin Baggage:</strong> {formatWeight(data?.CabinBaggage) || "7KG"}
                                        </span>
                                        <span className="checkin-baggage">
                                          <img src="/img/checkin_bag.svg" alt="Cabin Baggage" className="baggage-icon" /><strong>Check-In Baggage:</strong> {formatWeight(data?.Baggage) || "15KG"}
                                        </span>
                                      </div>
                                    </div>
                                  </div>
                        ...
                        <div className="table-container">
                          ...
                          {isOpen && (
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                              <div className="bg-white p-4 rounded-lg  shadow-lg relative"
                                style={{ width: '700px' }}>
                                <h2 className="text-lg bg-purple font-semibold mb-3">Details</h2>
                          ...
                          {Segments?.some(seg => seg.Airline?.AirlineCode === '6E') ? (
                            <table className="styled-table">
                              ...
                              <tbody>
                                {(() => {
                                  ...
                                  const cancelItem = CancellationData?.Cancellation?.find(
                                    (item) => item.fare_name === fare_type
                                  );
                                  return cancelItem ? (
                                    <tr>
                                      <td>
                                        {cancelItem.fare_name} ({cancelItem.airline_short_name})
                                      </td>
                                      <td>{cancelItem.fee.replace(/INR/i, '₹')}</td>
                                    </tr>
                                  ) : (
                                    <tr>
                                      <td colSpan={2}>No cancellation policy found</td>
                                    </tr>
                                  );
                                })()}
                                ...
                                {(() => {
                                  const dateChangeItem = CancellationData?.Date_Change?.find(
                                    (item) => item.fare_name === fare_type
                                  );
                                  return dateChangeItem ? (
                                    <tr>
                                      <td>
                                        {dateChangeItem.fare_name}({dateChangeItem.airline_short_name})
                                      </td>
                                      <td>{dateChangeItem.fee.replace(/INR/i, '₹')}</td>
                                    </tr>
                                  ) : (
                                    <tr>
                                      <td colSpan={2}>No date change policy found</td>
                                    </tr>
                                  );
                                })()}
                              </tbody>
                            ...
                            <table className="styled-table">
                              ...
                              <tbody>
                                {(() => {
                                  const cancelItem = CancellationData1G?.CancelPenalty;
                                ...
                                {(() => {
                                  const cancelItem = CancellationData1G?.ChangePenalty
                                  const formatValue = (item) => {
                                    if (item?.Amount) {
                                      return item.Amount.includes('INR')
                                        ? item.Amount.replace(/INR/i, '₹')
                              ...
                              <AccordionDetails>
                                ...
                                <div className='' id=''>
                                  {passengerList.map((passenger, formIndex) => (
                                    <div key={formIndex} ref={el => (formRefs.current[formIndex] = el)}>
                                      <div id="totalPassenger" data-totalpassenger={formIndex + 1} />
                                      <input type="hidden" name="passengerkey[]" />
                                      <input type="hidden" name="passengercode[]" />
                                      ...
                                      <div className="booking-container1">
                                        ...
                                        <div className="booking-row">
                                          {passenger.type == 'Adult' && (
                                            <div className="booking-field booking-prefix">
                                              <label>Prefix<span className="mandatory-star">*</span></label>
                                              <div className="form-calendar1">
                                                <select
                                                  ...
                                                  disabled={bookingid}
                                                  style={{ pointerEvents: bookingid ? 'none' : 'auto', backgroundColor: bookingid ? '#f0f0f0' : 'white' }}
                                                  defaultValue={TaxivaxiPassengeDetails?.[formIndex]?.gender === "Female" ? "Mrs" : "Mr"}
                                                >
                                          ...
                                          <div className="booking-field booking-name">
                                            ...
                                            <input
                                              ...
                                              readOnly={bookingid}
                                              style={{ pointerEvents: bookingid ? 'none' : 'auto', backgroundColor: bookingid ? '#f0f0f0' : 'white' }}
                                              defaultValue={TaxivaxiPassengeDetails?.[formIndex]?.firstName ? TaxivaxiPassengeDetails[formIndex].firstName : ''}
                                            // defaultValue={
                                            ...
                                            // }
                                            />
                                            <span className="error-message adult_first_name-message" style={{ display: "none", color: "red", fontWeight: "normal" }}>
                                          ...
                                          <div className="booking-field booking-name">
                                            ...
                                            <input
                                              ...
                                              readOnly={bookingid}
                                              style={{ pointerEvents: bookingid ? 'none' : 'auto', backgroundColor: bookingid ? '#f0f0f0' : 'white' }}
                                              defaultValue={TaxivaxiPassengeDetails?.[formIndex]?.lastName ? TaxivaxiPassengeDetails[formIndex].lastName : ''}
                                            ...
                                            // }
                                            />
                                            <span className="error-message adult_last_name-message" style={{ display: "none", color: "red", fontWeight: "normal" }}>
                                            ...
                                            <div className="booking-field booking-gender">
                                              ...
                                              <div className="form-calendar1">
                                                <select
                                                  ...
                                                  disabled={bookingid}
                                                  style={{ pointerEvents: bookingid ? 'none' : 'auto', backgroundColor: bookingid ? '#f0f0f0' : 'white' }}
                                                  defaultValue={TaxivaxiPassengeDetails?.[formIndex]?.gender === "Female" ? 'F' : 'M'}
                                                >
                                        ...
                                        <div className="booking-row mb-0">
                                          {passenger.type == 'Adult' && (
                                            <>
                                              ...
                                              <div className="booking-field booking-email gap-0">
                                                <label>Email ID<span className="mandatory-star">*</span></label>
                                                <input
                                                  type="email"
                                                  name="email1"
                                                  data-index={formIndex}
                                                  ...
                                                  readOnly={bookingid}
                                                  style={{ pointerEvents: bookingid ? 'none' : 'auto', backgroundColor: bookingid ? '#f0f0f0' : 'white' }}
                                                  defaultValue={TaxivaxiPassengeDetails?.[formIndex]?.employee_email || ''}
                                                />
                                                <span className="error-message" style={{ display: "none", color: "red", fontWeight: "normal" }}>
                                              ...
                                              <div className="booking-field booking-mobile gap-0">
                                                <label>Mobile Number<span className="mandatory-star">*</span></label>
                                                <div className="mobile-input-wrapper" >
                                                  <PhoneInput
                                                    international
                                                    country={'in'}
                                                    value={phoneNumbers || (TaxivaxiPassengeDetails?.[0]?.employee_contact ? `+91${TaxivaxiPassengeDetails[0].employee_contact}` : '')}
                                                    // onChange={(value) => {
                                                    //   setPhoneNumbers(value);
                                                    // }}
                                                    ...
                                                    style={{ pointerEvents: bookingid ? 'none' : 'auto', backgroundColor: bookingid ? '#f0f0f0' : 'white' }}
                                                    // name="contact_details1"
                                                    defaultValue={TaxivaxiPassengeDetails?.[formIndex]?.employee_contact || ''}
                                                  />
                                                </div>
                                                {phoneError && (
                                          ...
                                          {/* {passenger.type !== 'Adult' && ( */}
                                          <div className="booking-field booking-gender">
                                            <label>Date Of Birth <span> {FlightType === "International" && "*"}
        </span></label>
                                            <div className='form-calendar' style={{ width: "32%" }}>
                                              <input
                                                type="date"
                                                name="adult_age[]"
                                                max={maxDate}
                                                ...
                                                defaultValue={
                                                  TaxivaxiPassengeDetails &&
                                                    ...
                                                    TaxivaxiPassengeDetails[formIndex]['date_of_birth']
                                                    ? TaxivaxiPassengeDetails[formIndex]['date_of_birth']
                                        ...
                                        {FlightType === 'International' && (
                                          <div className="booking-row mb-0">
                                            <div className="booking-field booking-name">
                                              <label>Passport No.<span className="mandatory-star">*</span></label>
                                              <input
                                                type="text"
                                              ...
                                              // }
                                              />
                                              <div className="error-message adult_passportNo-message" style={{ color: 'red', display: 'none' }}></div>
                                            </div>
                                            <div className="booking-field booking-gender">
                                              <label>Issued Country<span className="mandatory-star">*</span></label>
                                              ...
                                              <Select
                                                ...
                                                onChange={(selectedOption) => {
                                                  // Update state...
                                                  const error = document.querySelector('.adult_issuedcountry-message');
                                                  if (selectedOption) error.style.display = 'none';
                                                }}
                                                styles={{
                                                  control: (provided) => ({
                                                    ...provided,
                                              ...
                                              />
                                              <div className="error-message adult_issuedcountry-message" style={{ color: 'red', display: 'none' }}></div>
                                            </div>
                                            <div className="booking-field booking-gender">
                                              <label>Passport Expiry<span className="mandatory-star">*</span></label>
                                              <div className='form-calendar' style={{ width: "98%" }}>
                                                <input
                                                  type="date"
                                                ...
                                                // }
                                                />
                                                <div className="error-message adult_passportexpiry-message" style={{ color: 'red', display: 'none' }}></div>
                                              </div>
                                            </div>
                                          </div>
                                        )}
                                ...
                                <div className="" id="">
                                  ...
                                  >
                                    <div className="booking-form-i booking-form-i3">
                                      <label>GSTIN<span className="mandatory-star">*</span></label>
                                      <div className="input">
                                        <input
                                          type="text"
                                          name="gstin"
                                        ...
                                        />
                                        {errors.gstin && <div className="error-message" style={{ color: 'red' }}>{errors.gstin}</div>}
                                      </div>
                                    </div>
                                    ...
                                    <div className="booking-form-i booking-form-i3">
                                      ...
                                      <div className="input">
                                        <input
                                          ...
                                          placeholder=""
                                        />
                                        <div className="error-message company_name-message" style={{ color: 'red'}}>Please enter Name.</div>
                                      </div>

                                    </div>

                                    <div className="booking-form-i booking-form-i3">
                                      <label>GST Address<span className="mandatory-star">*</span></label>
                                      <div className="input">
                                        <input
                                          type="text"
                                          name="company_address"
                                          defaultValue={GstEntries.company_address}
                                          onChange={(e) => setGstForm({ ...gstForm, company_address: e.target.value })}
                                          placeholder=""
                                        />
                                        <div className="error-message company_address-message" style={{ color: 'red'}}>  Please enter GST Address.</div>
                                      </div>

                                    </div>

                                    <div className="booking-form-i booking-form-i3">
                                      <label>GST Contact<span className="mandatory-star">*</span></label>
                                      <div className="input">
                                        <input
                                          type="text"
                                          name="company_contact"
                                          defaultValue={GstEntries.company_contact}
                                          onChange={(e) => setGstForm({ ...gstForm, company_contact: e.target.value })}
                                          placeholder="" />
                                         {errors.company_contact && <div className="error-message" style={{ color: "red" }}>{errors.company_contact}</div>}
                                      </div>
                                    ...
                                    <div className="booking-form-i booking-form-i2">
                                      ...
                                      <div className="input">
                                        <input
                                          ...
                                          // }
                                          defaultValue={TaxivaxiPassengeDetails?.[0]?.employee_contact || ''}
                                        />
                                      </div>
                                      <span
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              {Optionalservice[0]?.Segment[0]?.OptionalServices?.Baggage ? ''
                                : Optionalservice[0]?.Baggage ? ''
                                  : (
                                    <span className='extradisabled mb-0'>Extra Baggage selections are not allowed</span>
                                  )
                              }
                                        ...
                                        {selectedBaggage.map((b, index) => {
                                          return (
                                            ...
                                            >
                                              ...
                                              <div>{replaceINRWithSymbol(b.baggage.TotalPrice)}</div>
                                            </div>
                                          );
                                        })}
                                      </div>
                          ...
                          <AccordionDetails className="flex gap-6  ">
                                ...
                                .map((passenger, index) => {
                                  ...
                                  return (
                                    <button
                                      key={index}
                                      type="button"
                                      className={`p-3 rounded-lg border text-left ${index === selectedPassengerIndex ? 'border-violet-500 bg-violet-50' : 'border-gray-300'
                                        } text-gray-700`}
                                      onClick={() => { setSelectedPassengerIndex(index); setSelectedPassengerKey(passenger.Key) }}
                                    >
                            ...
                            <div className='w-3/4'>
                              <div className="flex flex-col   h-[400px] overflow-y-auto">
                                ...
                                {(() => {
                                  const passengerService = Optionalservice?.find(service => service.Key === selectedPassengerKey);

                                  const segmentIndex = selectedSegmentKey;
                                  const segmentData = passengerService?.["Segment"]?.[segmentIndex];
                                  const baggage = [];
                                  if (passengerService?.Segment.length > 1) {
                                    baggage.push(passengerService?.Baggage);
                                  } else {
                                    baggage.push(segmentData?.OptionalServices?.Baggage);
                                  }
                                  ...
                                  if (modBag != undefined) {
                                    return modBag.map((Baggage, idx) => {
                                      ...
                                      const quantity = currentSelected?.quantity || 0;
                                      return (
                                        <div
                                          key={idx}
                                        ...
                                        >
                                          <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center">
                                              <img src="/img/checkin_bag.svg" alt="Baggage Icon" className="w-6 h-6" />
                                            </div>
                                            <div>
                                              <div className="font-semibold text-sm">{Baggage?.DisplayText}</div>
                                            </div>
                                          </div>

                                          <div className="flex items-center gap-4">
                                            <div className="text-sm font-semibold text-black">{replaceINRWithSymbol(Baggage?.TotalPrice)}</div>
                                            <div className="flex items-center gap-2 baggageoptionbuttons">
                                              {quantity == 0 ? (
                                                <button
                                                  ...
                                                  onClick={() => handleAddBaggage(Baggage, idx)} // pass index
                                                >
                                                  Add
                                                </button>
                                              ) : (
                                                <>
                                                  <button
                                                    ...
                                                    onClick={() => handleRemoveBaggage(idx)} // pass index
                                                  >
                                                    -
                                                  </button>
                        ...
                        <Accordion
                          ...
                          onChange={(event, isExpanded) => {
                            if (Optionalservice[0]?.Segment[0]?.OptionalServices?.MealOrBeverage) {
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              {!Optionalservice[0]?.Segment[0]?.OptionalServices?.MealOrBeverage ? (
                                // <>
                                        ...
                                        {selectedMeals.map((meals, index) => {
                                          return (
                                            ...
                                            >
                                              ...
                                              <div>{replaceINRWithSymbol(meals.meal.TotalPrice)}</div>
                                            </div>
                                          );
                                        })}
                                      </div>
                          ...
                          <AccordionDetails className="flex gap-6">
                            ...
                            <div className="w-1/4 border-r pr-4 flex flex-col gap-3">
                              {PassengerData
                                .filter((passenger) => passenger.type !== 'Infant') // Adjust key if needed
                                .map((passenger, index) => {
                                  const passengerService = Optionalservice?.find(service => service.Key === selectedPassengerKey);
                                  const segmentIndex = selectedSegmentKey;
                                  const segmentData = passengerService?.["Segment"]?.[segmentIndex];
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <span className="text-xs text-gray-500">
                                        {passengerMeal
                                          ? `Selected: ${passengerMeal.meal.DisplayText || 'No Meal'}`
                                          : 'Meal: Not Selected'}
                                      </span>
                                    </button>
                                  );
                            ...
                            <div className="w-3/4">
                              {Optionalservice?.length > 0 && Optionalservice[0]?.Segment?.length > 0 && (
                                <div className="mb-4">
                                  <Tabs
                                  ...
                                  // scrollButtons="auto"
                                  >
                                    {Optionalservice[0].Segment.map((segment, index) => {
                                      const segmentData = segment;

                                      const origin = segmentData?.Origin?.airport_municipality || 'Unknown Origin';
                                      const destination = segmentData?.Destination?.airport_municipality || 'Unknown Destination';
                                ...
                                <div className="flex flex-col gap-2 max-h-[400px] overflow-y-auto">
                                  {
                                    (() => {
                                      ...
                                      const segmentData = passengerService?.["Segment"]?.[segmentIndex];
                                      const segmentKey = segmentData?.Key;
                                      const meals = segmentData?.OptionalServices?.MealOrBeverage || [];
                                          ...
                                          {meals.length == 0 ? (
                                            ...
                                            >
                                              <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                  <div className="text-sm font-medium">{meal?.DisplayText || 'Meal Option'}</div>
                                                </div>
                                              </div>

                                              <div className="flex items-center gap-6">
                                                <div className="font-semibold text-sm">
                                                  {replaceINRWithSymbol(meal?.TotalPrice) || '0.00'}
                                                </div>

                                                <button
                                                  type="button"
                                                  className={`px-4 py-1 border rounded-xl text-sm ${selectedMeals.some(
                                                    (m) =>
                        ...
                        <form >
                                                ...
                                                <ol className="cabin fuselage">
                                                  ...
                                                  {/* {(() => {
                                                    ...
                                                    if (!segmentData || !Array.isArray(segmentData.Rows) || segmentData.Rows.length === 0) {
                                                      return <div className="text-center text-gray-500 font-semibold py-4 px-4">Seat Reservation Not Available</div>;
                                                    }
                                                    ...
                                                    segmentData?.Rows?.forEach((row) => {
                                                      ...
                                                      validSeats.forEach((seat) => {
                                                        const seatLetter = seat.SeatCode.split('-')[1]; // Extract A, B, etc.
                                                        rowMap[rowNo][seatLetter] = {
                                                          ...seat,
                                                          rowNo,
                                                          seatLetter,
                                                          seatCode: seat.SeatCode,
                                                          price: seat.Paid === "true" ? seat.TotalPrice : 0, // Placeholder price
                                                      ...
                                                      <>
                                                        {sortedRowNos.map((rowNo, rowIndex) => (
                                                          <li key={rowIndex} className={`row row--${rowNo}`}>
                                                            <ol className={`seats border-0 olrow${sortedSeatLetters.length}`} type="A">
                                                              <li className="seat border-0">{rowNo}</li>
                                                              {sortedSeatLetters.map((seatLetter, seatIndex) => {
                                                                const seat = rowMap[rowNo][seatLetter];
                                                                if (!seat) return <li key={seatIndex} className="seat border-0 empty-seat"></li>;
                                                                ...
                                                                return (
                                                                  <li key={seatIndex} className="seat border-0">
                                                                    <input
                                                                      ...
                                                                      id={seatCode}
                                                                      value={
                                                                        seatPrice > 0 ? `${seatCode}_${seatPrice}` : `free ${seatCode}`
                                                                      }
                                                                      disabled={isUnavailable || selectedSeats.some(
                                                                        s =>
                                                                          s.seatCode === seatCode &&
                                                                    ...
                                                                    <label
                                                                      ...
                                                                      title={`[${seatCode}] ${seatPrice} ${isUnavailable ? 'unavailable' : 'available'}`}
                                                                    ></label>
                                                  ...
                                                  {(() => {
                                                    ...
                                                    segmentData?.Rows?.forEach((row) => {
                                                      ...
                                                      validSeats.forEach((seat) => {
                                                        const seatLetter = seat.SeatCode.split('-')[1]; // A, B, etc.
                                                        rowMap[rowNo][seatLetter] = {
                                                          ...seat,
                                                          rowNo,
                                                          seatLetter,
                                                          ...
                                                          <li key={rowIndex} className={`row row--${rowNo}`}>
                                                            <ol className={`seats border-0 olrow${sortedSeatLetters.length}`} type="A">
                                                              ...
                                                              {sortedSeatLetters.map((seatLetter, seatIndex) => {
                                                                const seat = rowMap[rowNo][seatLetter];
                                                                if (!seat) {
                                                                  return <li key={seatIndex} className="seat border-0 empty-seat"></li>;
                                                                }
                                                                ...
                                                                return (
                                                                  <li key={seatIndex} className="seat border-0">
                                                                    ...
                                                                    <label
                                                                      htmlFor={seatCode}
                                                                      className={`${numericSeatPrice > 0 ? 'paid' : 'free'} ${isUnavailable ? 'unavailable' : 'available'}`}
                                                                      title={`[${seatCode}] ${(!IsLCC && numericSeatPrice > 0)
                          ...
                          <div>
                            ...
                            <label className='confirmtocontinue'>I confirm that I have read and I accept the <a href="#">Fare Rules</a> , the <a href="#">Privacy Policy</a> , the <a href="#">User Agreement</a> and <a href="#">Terms of Service</a> of Taxivaxi</label>
                            {showError && (
                              <div style={{ color: 'red', marginBottom: '5px', fontSize: "11px" }}>
                  ...
                  <div className="chk-lines">
                    <div className="chk-line chk-fligth-info">
                      <div className="chk-departure">
                        <span />
                        <b>
                          {handleweekdatemonthyear(FlightData?.Origin?.OriginAirport?.DepTime)}
                        </b>
                      </div>
                      ...
                      <div className="chk-arrival">
                        <span />
                        <b>
                          {handleweekdatemonthyear(FlightData?.Destination?.DestinationAirport?.ArrTime)}
                        </b>
                      </div>
                      <div className="clear" />
                    </div>
                  ...
                  <div className="chk-details">
                    ...
                    <div className="chk-detais-row">
                      <div className="chk-line">
                        ...
                        <span className="chk-r">
                          {FlightData?.Origin?.OriginAirline?.AirlineName === FlightData?.Destination?.DestinationAirline?.AirlineName ? (
                            <>
                              {FlightData?.Origin?.OriginAirline?.AirlineName}
                            </>
                          ) : (
                            <>
                              {FlightData?.Origin?.OriginAirline?.AirlineName} <br />{FlightData?.Destination?.DestinationAirline?.AirlineName}
                            </>
                          )}
                        </span>
                        <div className="clear" />
                      ...
                      <div className="chk-line">
                        ...
                        <span className="chk-r">{responseData?.CabinClass}</span>
                        <div className="clear" />
                      </div>
                        ...
                        <div className="chk-line">
                          ...
                          <span className="chk-r">
                            <span className="chk-r">
                              {
                                (() => {
                                  const airPricingInfos = Array.isArray(FareData?.AirPricingInfo)
                                    ? FareData.AirPricingInfo
                                    : FareData?.AirPricingInfo
                                      ? [FareData.AirPricingInfo]
                                      : [];

                                  const adultPricing = airPricingInfos.find(info => {
                                    const passengerTypes = info?.['air:PassengerType'];
                                    const passengerTypeArray = Array.isArray(passengerTypes)
                                      ? passengerTypes
                                      : passengerTypes
                                        ? [passengerTypes]
                                        : [];

                                    return passengerTypeArray.some(pt => pt?.$?.Code === 'ADT');
                                  });

                                  const basePriceStr = adultPricing?.$?.BasePrice || 'INR0.00';
                                  const basePrice = parseFloat(basePriceStr.replace('INR', '').replace(',', '')) || 0;
                                  const adultCount = Number(responseData?.Passenger_info?.Adult) || 0;
                            ...
                            <span className="chk-r">
                              {
                                (() => {
                                  ...
                                  const adultPricing = airPricingInfos.find(info => {
                                    ...
                                    return passengerTypeArray.some(pt => pt?.$?.Code === 'CNN');
                                  });

                                  const basePriceStr = adultPricing?.$?.BasePrice || 'INR0.00';
                                  const basePrice = parseFloat(basePriceStr.replace('INR', '').replace(',', '')) || 0;
                                  const childCount = Number(responseData?.Passenger_info?.Child) || 0;
                              ...
                              {/* ₹ {PerPassFareData.find(item => item.PassengerType === 3)?.BaseFare?.toFixed(2) || '0.00'} */}
                              {
                                (() => {
                                  ...
                                  const adultPricing = airPricingInfos.find(info => {
                                    ...
                                    return passengerTypeArray.some(pt => pt?.$?.Code === 'INF');
                                  });

                                  const basePriceStr = adultPricing?.$?.Fees || 'INR0.00';
                                  const basePrice = parseFloat(basePriceStr.replace('INR', '').replace(',', '')) || 0;
                                  const infantCount = Number(responseData?.Passenger_info?.Infant) || 0;
                      ...
                      <div className="chk-line">
                        ...
                        <span className="chk-r">
                          ₹ {FareData?.OtherCharges?.toFixed(2) || 0.00}
                        </span>
                        <div className="clear" />
                      </div>
                      <div className="chk-line">
                        ...
                        <span className="chk-r">
                          {replaceINRWithSymbol(FareData?.Fees) || 0.00}
                        </span>
                        <div className="clear" />
                      </div>
                    ...
                    <div className="chk-total">
                      ...
                      <div className="chk-total-r" style={{ fontWeight: 700 }}>
                        ...
                        {(() => {
                          const basePrice = parseFloat((FareData?.TotalPrice || '').replace(/[^\d.]/g, '')) || 0;
