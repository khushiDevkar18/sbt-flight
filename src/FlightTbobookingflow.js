    ...
    timeoutRef.current = setTimeout(() => {
      const FetchOptions = async () => {
        ...
        try {
          ...
          if (Data.status) {
            if (responseData.FareRule_Response?.FareRules.length == 0 || responseData.FareQuote_Response?.Error?.ErrorCode != 0 || responseData?.FlightSSR_Response?.Error?.ErrorCode != 0) {
              ...
              if (responseData.FareRule_Response.FareRules) {
                const policy = responseData.FareRule_Response.FareRules[0].FareRuleDetail;
                // console.log(policy);
                setpolicy(policy);
              }
              if (responseData.FareQuote_Response) {
                const trace_id = responseData.FareQuote_Response.TraceId;
                const resultindex = responseData.FareQuote_Response.Results.ResultIndex;
              ...
              if (responseData.FareQuote_Response.Results.MiniFareRules) {
                const cancellation = responseData.FareQuote_Response.Results.MiniFareRules[0];
                // console.log(cancellation)
                setcancellationData(cancellation)
              }
              if (responseData.FareQuote_Response?.Results.Segments) {
                const segment = responseData.FareQuote_Response.Results.Segments[0];
                SetSegment(segment);
              }
              ...
              if (responseData.FareQuote_Response?.Results.FareBreakdown) {
                const PerFare = responseData.FareQuote_Response?.Results.FareBreakdown;
                setPerPassFareData(PerFare);
              }
              if (responseData.FareQuote_Response?.Results.IsFreeMealAvailable) {
                const meal = responseData.FareQuote_Response.Results.IsFreeMealAvailable;
                if (meal === true) {
                  setismealfree(true);
                }
              }
              ...
              if (responseData?.FlightSSR_Response?.SeatDynamic) {
                const seatdata = responseData.FlightSSR_Response?.SeatDynamic?.[0]?.SegmentSeat;
                // console.log(seatdata)
                setSeatdata(seatdata)
              }
              if (responseData.FlightSSR_Response?.MealDynamic) {
                const mealdata = responseData.FlightSSR_Response?.MealDynamic[0];
...
if (FlightType === "International") {
  ...
  if (passportExpiryInput && !passportExpiryInput.value.trim()) {
    ...
    const Address1 = form.querySelector('input[name="adult_address_line_1[]"]')?.value || "";
    const Address2 = form.querySelector('input[name="adult_address_line_2[]"]')?.value || "";
    const city = form.querySelector('input[name="adult_city[]"]')?.value || "";
    ...
    const passengerData = {
      ...
      contact: ["Child", "Infant"].includes(passenger.type) ? leadContact : contact,
      gender: genderVal,
      Address1: ["Child", "Infant"].includes(passenger.type) ? leadAddress1 : Address1,
      Address2: ["Child", "Infant"].includes(passenger.type) ? leadAddress2 : Address2,
      city: ["Child", "Infant"].includes(passenger.type) ? LeadCity : city,
      passportno: form.querySelector('input[name="adult_passportNo[]"]')?.value || "",
      passportexpiry: form.querySelector('input[name="adult_passportexpiry[]"]')?.value || "",
      issuedcountry: form.querySelector('input[name="adult_issuedcountry[]"]')?.value || "",
  ...
  const totalMealPrice = selectedMeals.reduce((total, item) => total + (Number(item.meal.Price) || 0), 0);
  const totalBaggagePrice = selectedBaggage.reduce((total, item) => total + (Number(item.baggage.Price) || 0), 0);
  const totalSeatPrice = selectedSeats.reduce((total, item) => total + (Number(item.seatPrice) || 0), 0);
  ...
  const SaveGstDetails = () => {
    ...
    const updatedGstForm = {
      ...
      company_address: form.querySelector('input[name="company_address"]')?.value || '',
      company_contact: form.querySelector('input[name="company_contact"]')?.value || '',
      company_email: form.querySelector('input[name="company_email"]')?.value || '',
    };
                                ...
                                CancellationData
                                  ...
                                  .map((item, index) => {
                                    ...
                                    const fromDate = new Date(
                                      new Date(FlightData?.Origin?.OriginAirport?.DepTime).getTime() - fromHours * 60 * 60 * 1000
                                    );
                                    const toDate = toHours
                                      ? new Date(
                                        new Date(FlightData?.Origin?.OriginAirport?.DepTime).getTime() - toHours * 60 * 60 * 1000
                                      )
                                      : null;
                                      ...
                                      <div className="booking-container1">
                                        ...
                                        <div className="booking-row">
                                          <div className="booking-field booking-gender">
                                            <label>Date Of Birth<span className="mandatory-star">
          {FlightType === "International" && "*"}
        </span></label>
                                            <div className='form-calendar' style={{ width: '99%' }}>
                                              <input
                                                type="date"
                                                name="adult_age[]"
                                                max={maxDate}
                                              ...
                                              />
                                            <span className="error-message adult_age-message" style={{ display: "none", color: "red", fontWeight: "normal" }}>
                                          ...
                                          {passenger.type == 'Adult' && (
                                            <div className="booking-field booking-name">
                                              <label>Address Line 1 <span className="mandatory-star">
          {FlightType === "International" && "*"}
        </span></label>
                                              <input
                                                type="text"
  ...
  style={{
    display: FlightType === "Domestic" ? "none" : "none",
    color: "red",
    fontWeight: "normal"
  }}
>
  {FlightType === "International" && "Please Enter The Address Line 1."}
</span>
                                        ...
                                        {passenger.type == 'Adult' && (
                                          <div className="booking-row">
                                            <div className="booking-field booking-name">
                                              <label>Address Line 2<span className="mandatory-star">
          {FlightType === "International" && "*"}
        </span></label>
                                              <input
                                                type="text"
...
>
  {FlightType === "International" && "Please Enter The Address Line 2."}
</span>
                                            </div>
                                            <div className="booking-field booking-name">
                                              <label>City<span className="mandatory-star">
          {FlightType === "International" && "*"}
        </span></label>
                                              <input
                                                type="text"
...
>
  {FlightType === "International" && "Please Enter City."}
</span>
                                            </div>
                                            ...
                                            <div className="booking-field booking-email">
                                              ...
                                              />
                                              <span className="error-message email1-message">Please Enter The Email ID.</span>
                                            </div>

                                            <div className="booking-field booking-mobile">
                                              <label>Mobile Number<span className="mandatory-star">*</span></label>
                                              <div className="mobile-input-wrapper" style={{ width: "32%" }} >
                                                <PhoneInput
                                                  international
                                                  country={'in'}
                                    ...
                                    <div className="booking-form-i booking-form-i2">
                                      ...
                                      <div className="input">
                                        <input
                                          ...
                                          placeholder=""
                                          // readOnly={bookingid}
                                          onChange={(e) => setGstForm({ ...gstForm, company_email: e.target.value })}
                                          defaultValue={GstEntries.company_email}
                                        />
                                        ...
                                        <div className="modal-dialog modal-md">
                                          <div className="modal-content rounded-md px-4 py-2" style={{ maxWidth: '605px', margin: 'auto' }}>
                                            ...
                                            <div className="flex justify-center gap-4 mt-1 pb-2">
                                              ...
                                              <button
                                                type="button"
                                                onClick={() => { setAccordion4Expanded(true); setAccordion5Expanded(false); setShowModal(false) }}
                        ...
                        <Accordion expanded={accordion4Expanded}
                          ...
                          <AccordionDetails className="flex gap-6  ">
                            <div className="w-1/4 border-r pr-4 flex flex-col gap-3 ">
                              {PassengerData.filter((passenger) => passenger.type !== 'Infant').map((passenger, index) => {
                                const passengerMeal = selectedMeals.find(
                                  (m) => m.passengerIndex === index && m.segmentKey === selectedSegmentKey
                                );
                                    ...
                                    <span className="text-xs text-gray-500">
                                      Baggage: {
                                        selectedBaggage
                                          ...
                                          .map((b) => `${BaggageData[b.baggageIndex]?.Weight}KG x${b.quantity}`)
                                          .join(', ') || 'Not Selected'
                                      }
                                    </span>
                                  </button>
                            ...
                            <div className='w-3/4'>
                              <div className="flex flex-col   h-[400px] overflow-y-auto">
                                {BaggageData.map((data, baggageIndex) => {
                                  ...
                                  const quantity = currentBaggage?.quantity || 0;
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="flex items-center gap-4">
                                        <div className="text-sm font-semibold text-black">₹{data.Price}</div>
                                          ...
                                          {quantity === 0 ? (
                                            ...
                                            <>
                                              <button
                                                ...
                                                onClick={() => handleRemoveBaggage(baggageIndex)}
                                              >
                                                -
                                              </button>
                                        ...
                                        {selectedMeals.map((meals, index) => {
                                          return (
                                            <div
                                              ...
                                              className="flex justify-between items-center gap-4"
                                            >
                                              <div>{meals.meal.AirlineDescription}</div>
                              ...
                              {PassengerData.filter((passenger) => passenger.type !== 'Infant').map((passenger, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <br />
                                    <span className="text-xs text-gray-500">
                                      {passengerMeal ? `Selected: ${passengerMeal.meal.AirlineDescription || 'No Meal'}` : 'Meal: Not Selected'}
                                    </span>
                                  </button>
                                );
                              })}
                              ...
                              <div className="w-3/4">
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-col gap-2 max-h-[400px] overflow-y-auto">
                                    {MealData.filter(item => `${item.Meal.Origin}-${item.Meal.Destination}` === selectedSegmentKey)
                                      .map((meal, idx) => {
                                        return (
                                          <div
                                            key={idx}
                                          ...
                                          >
                                            <div className="flex items-center gap-2">
                                              ...
                                              <div>
                                                <div className="text-sm font-medium">{ismealfree ? meal.Meal.Description : meal.Meal.AirlineDescription || 'No Meal'}</div>
                                              </div>
                                            </div>

                                            <div className="flex items-center gap-6">
                                              <div className="font-semibold text-sm">
                                                ₹{ismealfree ? 'Free' : meal.Meal.Price}
                                              </div>
                                    ...
                                    <div className='seatleftul'>
                                      {PassengerData.filter((passenger) => passenger.type !== 'Infant').map((passenger, index) => {
                                        ...
                                        const selectedSeat = selectedSeats.find(
                                          s => s.passengerIndex === index && s.segmentKey === CurrentSegmentCode
                                        );
                                      ...
                                      <div className='card-body' style={{ padding: '0px' }}>
                                        <div className="seat_selection">
                                          ...
                                          <>
                                            ...
                                            {SeatData.length > 1 && currentFlightIndex > 0 && (
                                              <button type="button" className='seatprevbutton' onClick={handlePrev}>{'<<'}</button>
                                            )}
                                            {SeatData.length > 1 && currentFlightIndex < SeatData.length - 1 && (
                                            ...
                                            <div style={{ display: 'block' }}>
                                              ...
                                              <div className="plane passenger">
                                                <div className="cockpit">
                                                  <h1>
                                                    {currentSegment.OriginAirport.airport_municipality || 'Origin'}
                                                    <br />
                                                    <span className="apiairportname1">{(currentSegment.OriginAirport.airport_name)}</span>
                                                    <br />
                                                    <span className="brcockpit">TO</span>
                                                    <br />
                                                    {currentSegment.DestinationAirport.airport_municipality || 'Destination'}
                                                    <br />
                                                    <span className="apiairportname1">{(currentSegment.DestinationAirport.airport_name)}</span>
                                                    <br />
                                                    <span className="brcockpit1">
                                                      (Flight Number: {currentSegment.RowSeats[0].Seats[0].FlightNumber}) -
                                                      <span className="equipmentno">
                                                        {currentSegment.RowSeats[0].Seats[0].CraftType?.split('-')[0] || 'Equip'}
                                                      </span>
                                                    </span>
                                                  </h1>
                                                </div>
                                                ...
                                                <ol className="cabin fuselage">
                                                  {(() => {
                                                    ...
                                                    currentSegment?.RowSeats?.forEach((row) => {
                                                      const validSeats = row.Seats.filter((s) => s.SeatNo);
                                                      if (!validSeats.length) return;

                                                      const rowNo = validSeats[0].RowNo;
                                                      if (!rowNo) return;

                                                      rowMap[rowNo] = {};
                                                      validSeats.forEach((seat) => {
                                                        rowMap[rowNo][seat.SeatNo] = seat;
                                                        seatLettersSet.add(seat.SeatNo);
                                                      });
                                                    });
                                                    ...
                                                    return (
                                                      <>
                                                        ...
                                                        <li className="row row--header">
                                                          <ol className={`seatsolrow${sortedSeatLetters.length}`}>
                                                          ...
                                                          <li key={rowIndex} className={`row row--${rowNo}`}>
                                                            <ol className={`seats border-0 olrow${sortedSeatLetters.length}`} type="A">
                                                              ...
                                                              {sortedSeatLetters.map((seatLetter, seatIndex) => {
                                                                ...
                                                                const isUnavailable = seat.AvailablityType === 3;
                                                                ...
                                                                return (
                                                                  <li key={seatIndex} className="seat border-0">
                                                                    ...
                                                                    <label
                                                                      htmlFor={seatCode}
                                                                      className={`${seatPrice > 0 ? 'paid' : 'free'} ${isUnavailable ? 'unavailable' : 'available'
                                                                        }`}
                                                                      title={`[${seatCode}] ₹${seatPrice}`}
                                                                    ></label>
                                                                    <span className="tooltip">
                                                                      {isUnavailable ? 'Unavailable' : 'Available'} [{seatCode}]
                                                                      ₹{seatPrice}
                                                                    </span>
                                                                  </li>
                                                                );
                  ...
                  <div className="chk-details">
                    ...
                    <div className="chk-detais-row">
                      ...
                      <div className="chk-line">
                        ...
                        <span className="chk-r">
                          ₹ {FareData?.OtherCharges?.toFixed(2)}
                        </span>
                        <div className="clear" />
                      </div>
